...
describe('calculate basic stats', () => {
  ...
  it('start', done => {
    given(() => [
      ...
      .then('stats.calculated', (topic: any, stats: any) => {
  ...
  function given(publications: () => any[]) {
    publish('board.ready', {}) // necessary to initialize stats
    let expectedEvents = publications();
    let callCounter = 0;
    return {
      then: (message: string, f: (topic: any, subject: any) => void) => {
        subscribe(message, (topic: any, subject: any) => {
          callCounter++;
